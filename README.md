# 📌 Programas em Arduino – Crislei

Este repositório reúne todos os meus programas desenvolvidos em **Arduino**, focados em lógica e aplicações práticas.

---

## 🏫 Sobre

Os programas foram desenvolvidos durante o **curso de Redes** na **ETEC**, sob a orientação do professor **Joel Coutinho**.  
Agradeço pela direção e ensinamentos que contribuíram para meu aprendizado na área de programação de microcontroladores.

---

## 👨‍🏫 Professor

**Joel Coutinho**  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Joel%20Coutinho-blue?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/joel-coutinho-b1b32937/)

---

## 📂 Conteúdo

Cada arquivo contém:
- Código-fonte em Arduino (.ino)

---

## 🛠 Tecnologias

- **Arduino IDE**  
- **Linguagem C/C++ para Arduino**

---

## 📬 Contato

- [![LinkedIn](https://img.shields.io/badge/LinkedIn-Crislei%20Jenuino-blue?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/crislei-jenuino/)
- ✉️ Email: crisleikeli@hotmail.com
